...
class EmbeddingLayer(AbstractLayer):
    def __init__(self, emb_dim:int, vocab_size:int, padding_idx:int=0, pretrained_mat=None, requires_grad:bool=False):
        super(Embeddings, self).__init__()
        self.emb_dim = emb_dim
        self.vocab_size = vocab_size
        self.freeze = not requires_grad

        self.embeddings = nn.Embeddings(self.vocab_size, self.emb_dim, padding_idx=padding_idx)
